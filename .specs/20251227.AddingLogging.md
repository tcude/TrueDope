...
Currently, the TrueDope backend uses Serilog but only writes logs to the console (stdout). Docker captures these logs, but they're mixed with other containers on the host and not easily accessible for Splunk.
