...
| Phase 2 | iOS Manual Measurement UI (Online) | **COMPLETE** |
| Phase 3 | Integration & Polish | **PARTIAL** |
...
The MVP delivers a fully functional manual measurement tool that works with ANY target, both online and offline. Auto-detection features are future enhancements only.
...
- `TrueDope.Api/Controllers/GroupMeasurementsController.cs`
- `TrueDope.Api/Services/GroupMeasurementCalculator.cs`
- `TrueDope.Api/Migrations/20251228160430_AddGroupMeasurement.cs`

**Deliverables:**
- ✅ Backend can store and calculate measurements from hole positions
- ✅ API endpoints for CRUD operations on measurements
- ✅ Image upload endpoint for original/annotated images

---

### Phase 2: iOS Manual Measurement UI (Online) ✅ COMPLETE
**Scope:** Camera capture, manual calibration, hole marking (online mode)

**Completed Files:**
- `TrueDope-iOS/Models/GroupMeasurement.swift` - Models and DTOs
- `TrueDope-iOS/Core/Services/GroupMeasurementService.swift` - API service
- `TrueDope-iOS/Features/GroupMeasurement/ViewModels/GroupMeasurementViewModel.swift` - 5-step workflow
- `TrueDope-iOS/Features/GroupMeasurement/Views/` - UI components

**Deliverables:**
- ✅ 5-step measurement flow: Capture → Calibration → Mark Holes → Metadata → Review
- ✅ Manual calibration (mark reference distance)
- ✅ Tap-to-mark hole interface with drag-to-adjust
- ✅ Local annotated image generation
- ✅ Metrics calculated via API and displayed
- ✅ Image upload (non-blocking on failure)

---

### Phase 3: Integration & Polish ⚠️ PARTIAL
**Scope:** Connect to existing flows, image handling, UX refinement

**What's Done:**
- ✅ GroupMeasurementService integrated with API
- ✅ GroupMeasurement model with full metrics
- ✅ Annotated image generation with caliber-scaled markers

**What's Missing:**
- ❌ Integration into `SessionDetailView` (add to existing group)
- ❌ "Measure Group" entry point on GroupEntry cards
- ❌ Display measurement details in group detail view
- ❌ Edge case handling polish

**Files to Create/Modify:**
- Modify `SessionDetailView.swift` - Add "Measure Group" button
- Modify `GroupEntryCard.swift` - Show measurement indicator
- Create `GroupMeasurementDetailView.swift` - Display full metrics

---

### Phase 4: Offline Support 🆕 NOT STARTED
**Scope:** Enable full group measurement workflow while offline with on-device calculations

**Why This Phase:**
- All calculation logic is pure math - no server dependency needed
- Existing `PendingGroupMeasurement` model already stores hole positions and bullet diameter
- Offline session infrastructure (`OfflineSessionQueue`, `PendingSession`) is in place
- Images can already be stored locally via `queueImage()`

**Key Insight:** The backend's `GroupMeasurementCalculator` logic can be replicated on-device in Swift. The only server interaction needed is sync (upload data + images when back online).

#### Phase 4a: Local Metrics Calculator
**Scope:** Port backend calculation logic to Swift

**Tasks:**
1. Create `LocalGroupMetricsCalculator.swift` with pure Swift implementation:
   - `calculateExtremeSpread(holes:bulletDiameter:)` → Decimal (inches)
   - `calculateMeanRadius(holes:)` → Decimal (inches)
   - `calculateHorizontalSpread(holes:bulletDiameter:)` → Decimal
   - `calculateVerticalSpread(holes:bulletDiameter:)` → Decimal
   - `calculateStandardDeviations(holes:)` → (radial, horizontal, vertical)
   - `calculateCEP50(horizontalStdDev:verticalStdDev:)` → Decimal
   - `calculateCentroid(holes:)` → HolePosition
   - `inchesToMoa(_:atYards:)` → Decimal

2. Expand `PendingGroupMeasurement` computed properties to use calculator:
   - Add `meanRadius`, `radialStdDev`, `horizontalStdDev`, `verticalStdDev`, `cep50`
   - Add `extremeSpreadMoa(atYards:)`, `meanRadiusMoa(atYards:)`

**Files to Create:**
- `TrueDope-iOS/Core/Utilities/LocalGroupMetricsCalculator.swift`

**Files to Modify:**
- `TrueDope-iOS/Models/PendingSession.swift` (expand `PendingGroupMeasurement`)

---

#### Phase 4b: Offline Measurement ViewModel
**Scope:** Create offline-capable version of measurement flow

**Tasks:**
1. Create `OfflineGroupMeasurementViewModel.swift`:
   - Same 5-step flow as online version
   - Uses `LocalGroupMetricsCalculator` instead of API
   - Saves to `PendingGroupMeasurement` instead of server
   - Stores images via `OfflineSessionQueue.queueImage()`
   - Updates parent `PendingGroupEntry` with calculated MOA values

2. Key differences from online `GroupMeasurementViewModel`:
   - No network calls during workflow
   - `save()` writes to local storage only
   - Metrics calculated locally, not from API response

**Files to Create:**
- `TrueDope-iOS/Features/GroupMeasurement/ViewModels/OfflineGroupMeasurementViewModel.swift`

---

#### Phase 4c: UI Integration for Offline
**Scope:** Add "Measure from Photo" to pending group forms

**Tasks:**
1. Modify `PendingGroupEntryForm.swift`:
   - Add "Measure from Photo" button alongside manual entry
   - Present `OfflineGroupMeasurementView` as sheet
   - On completion, populate form fields from measurement data

2. Create `OfflineGroupMeasurementView.swift`:
   - Wrapper that uses `OfflineGroupMeasurementViewModel`
   - Same UI as online version (can likely reuse view components)
   - Returns `PendingGroupMeasurement` to parent

3. Update `PendingSessionDetailView.swift`:
   - Show measurement data on group cards
   - Display annotated image thumbnail if available
   - Show calculated metrics (local values)

**Files to Create:**
- `TrueDope-iOS/Features/GroupMeasurement/Views/OfflineGroupMeasurementView.swift`

**Files to Modify:**
- `TrueDope-iOS/Features/Sessions/Components/PendingDataEntryForms.swift`
- `TrueDope-iOS/Features/Sessions/PendingSessionDetailView.swift`

---

#### Phase 4d: Sync Enhancement
**Scope:** Complete the sync flow for measurements and images

**Tasks:**
1. Modify `OfflineSessionQueue.syncSession()`:
   - After creating session, fetch full session details to get group IDs
   - For each group with measurement data:
     - Upload original image via `ImageService`
     - Upload annotated image via `ImageService`
     - Create measurement via `GroupMeasurementService.createMeasurement()`
   - Handle partial failures gracefully (measurement created but images failed)

2. Update sync error handling:
   - Track which groups succeeded/failed
   - Allow retry of failed image uploads
   - Log detailed sync progress

**Files to Modify:**
- `TrueDope-iOS/Core/Services/OfflineSessionQueue.swift` (lines 357-395)

---

#### Phase 4 Deliverables:
- Users can photograph targets and mark holes while offline
- All metrics calculated locally (identical to server calculations)
- Images stored locally until sync
- On sync: session created, images uploaded, measurements created
- Seamless experience - user doesn't need to know if online/offline
...
### Risk 6: Offline Calculation Drift (Phase 4)
**Issue:** Local calculations might differ from server calculations.

**Mitigation:**
- Use identical algorithm (port directly from C# to Swift)
- Store raw hole positions; recalculate on sync if needed
- Server is source of truth after sync
- Unit tests comparing local vs server results for same inputs
...
6. **Local calculations:** All metrics calculated on-device using identical algorithm to backend.
...
│   ├── Controllers/
│   │   ├── GroupMeasurementsController.cs  ✅ EXISTS
│   │   └── TargetsController.cs            (FUTURE)
│   ├── Data/Entities/
│   │   └── GroupMeasurement.cs             ✅ EXISTS
│   ├── DTOs/Sessions/
│   │   └── GroupMeasurementDtos.cs         ✅ EXISTS
│   ├── Services/
│   │   ├── GroupMeasurementCalculator.cs   ✅ EXISTS
│   │   └── TargetPdfGenerator.cs           (FUTURE)
│   └── Migrations/
│       └── 20251228160430_AddGroupMeasurement.cs ✅ EXISTS
│
└── TrueDope-iOS/TrueDope-iOS/
    ├── Models/
    │   ├── GroupMeasurement.swift          ✅ EXISTS
    │   └── PendingSession.swift            ✅ EXISTS (has PendingGroupMeasurement)
    ├── Core/Services/
    │   ├── GroupMeasurementService.swift   ✅ EXISTS
    │   └── OfflineSessionQueue.swift       ✅ EXISTS (needs sync enhancement)
    ├── Core/Utilities/
    │   └── LocalGroupMetricsCalculator.swift   🆕 PHASE 4a
    └── Features/GroupMeasurement/
        ├── Views/
        │   ├── GroupMeasurementView.swift      ✅ EXISTS
        │   ├── CaptureStepView.swift           ✅ EXISTS
        │   ├── HoleAdjustmentView.swift        ✅ EXISTS
        │   ├── MetadataEntryView.swift         ✅ EXISTS
        │   ├── MeasurementReviewView.swift     ✅ EXISTS
        │   ├── ManualCalibrationSheet.swift    ✅ EXISTS
        │   └── OfflineGroupMeasurementView.swift   🆕 PHASE 4c
        ├── ViewModels/
        │   ├── GroupMeasurementViewModel.swift     ✅ EXISTS
        │   └── OfflineGroupMeasurementViewModel.swift  🆕 PHASE 4b
        └── Components/
            ├── HoleMarker.swift                ✅ EXISTS
            ├── POAMarker.swift                 ✅ EXISTS
            └── ZoomableImageView.swift         ✅ EXISTS
```
...
| 2026-01-02 | 2.0 | Claude | Added implementation status, Phase 4 (Offline Support) |
