...
```
TrueDope-iOS/
├── TrueDope-iOS.xcodeproj
├── TrueDope-iOS/
│   ├── App/
│   │   ├── TrueDope_iOSApp.swift          # App entry point
│   │   └── AppState.swift                  # Global app state
│   │
│   ├── Core/
│   │   ├── Network/
│   │   │   ├── APIClient.swift             # HTTP client (Alamofire wrapper)
│   │   │   ├── APIEndpoints.swift          # Endpoint definitions
│   │   │   ├── APIError.swift              # Error types
│   │   │   └── APIResponse.swift           # Response wrapper types
│   │   │
│   │   ├── Auth/
...
│   │   ├── Services/
│   │   │   ├── RifleService.swift
│   │   │   ├── AmmunitionService.swift
│   │   │   ├── SessionService.swift        # Unified RangeSession service
...
│   │       ├── DateFormatters.swift        # ISO8601, display formats
│   │       ├── UnitConversions.swift       # MOA/MIL, Imperial/Metric
...
│   │   ├── Ammunition.swift
│   │   ├── AmmoLot.swift
│   │   ├── RangeSession.swift              # Unified session model
│   │   ├── DopeEntry.swift
│   │   ├── ChronoSession.swift
│   │   ├── VelocityReading.swift
│   │   ├── GroupEntry.swift
│   │   ├── SavedLocation.swift
...
│   │   │   ├── SessionCreateView.swift     # Unified session creation
│   │   │   ├── SessionCreateViewModel.swift
│   │   │   ├── SessionEditView.swift
│   │   │   └── Components/
│   │   │       ├── SessionCard.swift
│   │   │       ├── DopeEntryForm.swift
│   │   │       ├── DopeEntryRow.swift
│   │   │       ├── ChronoSessionForm.swift
│   │   │       ├── VelocityReadingRow.swift
│   │   │       ├── GroupEntryForm.swift
│   │   │       ├── GroupEntryRow.swift
│   │   │       ├── ConditionsSection.swift
│   │   │       └── LocationPicker.swift
│   │   │
│   │   ├── Rifles/
│   │   │   ├── RiflesListView.swift
│   │   │   ├── RiflesListViewModel.swift
│   │   │   ├── RifleDetailView.swift
│   │   │   ├── RifleDetailViewModel.swift
│   │   │   ├── RifleFormView.swift
│   │   │   ├── RifleFormViewModel.swift
│   │   │   └── Components/
│   │   │       └── RifleCard.swift
│   │   │
│   │   ├── Ammunition/
│   │   │   ├── AmmunitionListView.swift
│   │   │   ├── AmmunitionListViewModel.swift
│   │   │   ├── AmmunitionDetailView.swift
│   │   │   ├── AmmunitionDetailViewModel.swift
│   │   │   ├── AmmunitionFormView.swift
│   │   │   ├── AmmunitionFormViewModel.swift
│   │   │   ├── LotFormView.swift
│   │   │   └── Components/
│   │   │       ├── AmmoCard.swift
│   │   │       └── LotRow.swift
│   │   │
│   │   ├── Analytics/
│   │   │   ├── AnalyticsView.swift         # Dashboard with summary
│   │   │   ├── AnalyticsViewModel.swift
│   │   │   ├── DopeChartView.swift
│   │   │   ├── DopeChartViewModel.swift
│   │   │   ├── VelocityTrendsView.swift
│   │   │   ├── AmmoComparisonView.swift
│   │   │   ├── LotComparisonView.swift
...
│   │   │   ├── ProfileView.swift
│   │   │   ├── PreferencesView.swift       # Units, display preferences
│   │   │   ├── LocationsListView.swift     # Saved locations management
...
└── TrueDope-iOSTests/
    ...
    ├── Services/
    │   ├── RifleServiceTests.swift
    │   ├── SessionServiceTests.swift
    │   └── AmmunitionServiceTests.swift
    ├── ViewModels/
    │   ├── LoginViewModelTests.swift
    │   ├── SessionsListViewModelTests.swift
    │   └── RiflesListViewModelTests.swift
    └── Mocks/
        ├── MockAPIClient.swift
...
│                                                                   │
├───────────────────────────────────────────────────────────────────┤
│  🏠 Home  │  📋 Sessions  │  📊 Analytics  │  🔫 Gear  │  ⚙️ Settings  │
└───────────────────────────────────────────────────────────────────┘
```
...
| **Analytics** | `chart.xyaxis.line` | Analytics dashboard, DOPE charts, velocity trends, comparisons, cost analysis |
| **Gear** | `scope` | Rifle list, create, detail, ammunition management |
| **Settings** | `gearshape.fill` | Profile, preferences, locations, about |

**Why 5 tabs?**
- Analytics is a rich, frequently-accessed feature that deserves prominent placement
- Ammunition is rifle-adjacent (accessed from Gear tab or session creation)
...
```
Home
├── Recent Sessions → Session Detail
├── Quick Actions
│   ├── New Session → Session Create
│   └── View Analytics → Analytics Views
└── Summary Cards → Detailed Analytics

Sessions
├── Session List (filterable, searchable)
│   └── Session Detail
│       ├── Edit → Session Edit
│       └── Delete (confirmation)
└── + New Session → Session Create
    ├── Step 1: Date, Rifle, Location
    ├── Step 2: Conditions (manual or auto-fetch)
    └── Step 3: Data Entry (DOPE/Chrono/Groups)

Rifles
├── Rifle List (searchable)
│   └── Rifle Detail
│       ├── Edit → Rifle Form
│       ├── Delete (confirmation)
│       ├── View Sessions → Filtered Session List
│       └── Ammunition Section
│           ├── View All Ammo → Ammunition List
│           └── Ammo Detail → Lot Management
└── + New Rifle → Rifle Form

Settings
├── Profile (name, email)
├── Preferences
│   ├── Distance Units (yards/meters)
│   ├── Adjustment Units (MOA/MIL)
│   ├── Temperature (°F/°C)
│   ├── Pressure (inHg/hPa)
│   └── Velocity (fps/m/s)
├── Locations
...
- **Ammunition section:**
  - "View Ammunition" button → Ammunition list
- **Sessions section:**
  - Recent sessions for this rifle (3-5)
  - "View All Sessions" → filtered session list
- Edit button
- Delete button
...
struct UnitConverter {
    ...
    // Input conversion (user input → API format)
...
**Important**: Backend stores everything in base units (yards, °F, inHg, fps, MIL). iOS converts for display based on user preferences.
...
Backend handles HEIC→JPEG conversion. iOS can upload HEIC directly.
...
- [x] Create `UnitFormatter` utility (with Environment key for SwiftUI)
- [x] Update AppState with preferences and theme support
- [x] Create PreferencesView UI with live preview
- [x] Update ConditionsSection for temperature/pressure conversion
- [x] Update DopeEntryRow for distance and MIL/MOA primary swap
- [x] Update GroupEntryRow for distance conversion
- [x] Update SessionDetailView for distance in confirmations
- [x] Write UnitFormatter tests
- [x] Implement theme switching (System/Light/Dark)

### Phase 13.12: Home Dashboard
- [x] Create `DashboardView` + ViewModel
- [x] Implement recent sessions section
- [x] Implement quick actions
- [x] Implement summary stats
- [x] Pull-to-refresh

### Phase 13.13: Analytics Feature
- [x] Create `AnalyticsView` dashboard with navigation to all analytics views
- [x] Create `DopeChartView` with Swift Charts (rifle selector, date range filter)
- [x] Create `VelocityTrendsView` with Swift Charts (ammo/lot selector, date range filter)
- [x] Create `AmmoComparisonView` with multi-select and comparison charts
- [x] Create `LotComparisonView` with velocity/SD comparison charts
- [x] Create `CostAnalysisView` with spending breakdown charts
- [x] Add Analytics tab to bottom navigation bar (5 tabs total)
- [x] Fix iOS model DTOs to match backend API response structures
- [x] Fix API array parameter encoding for ASP.NET Core (noBrackets)
- [x] Fix backend DateTime UTC handling for PostgreSQL timestamp queries
- [x] Implement loading states, error handling, and empty state views for all analytics screens

### Phase 13.14: Polish & Error Handling
- [x] Implement network reachability monitoring (NetworkMonitor.swift with NWPathMonitor)
- [x] Add offline messaging (OfflineBanner.swift, disable save buttons when offline)
- [x] Review all error states (already well implemented)
- [x] Add haptic feedback where appropriate (HapticManager.swift - login, save, delete operations)
- [x] Review accessibility (Dynamic Type - fixed 4 hardcoded font sizes)
- [x] Dark mode verification (already tested and working)
- [x] Landscape support (already tested and working)

See [20251225.Phase13.14.Polish&ErrorHandling.md](./20251225.Phase13.14.Polish&ErrorHandling.md) for full implementation details.
...
| 2024-12-25 | Phase 13.13 completed: Analytics feature with all charts, added 5th tab for Analytics, fixed iOS model DTOs, API array encoding, and backend DateTime UTC handling |
