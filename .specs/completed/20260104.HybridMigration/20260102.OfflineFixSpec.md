...
### Task 4: Offline Indicators ✅ COMPLETE
- [x] Create or update `OfflineBanner` component
- [x] Add to `DashboardView`
- [x] Add to `SessionsListView`
- [x] Add to `GearTabView` (Rifles/Ammo)
- [x] Test indicators appear when offline

**Implementation Note:** Rather than adding the banner to each view individually, `OfflineBanner` was added once at the root level in `TrueDope_iOSApp.swift` (`RootView`), where it overlays all authenticated views automatically.

### Task 5: Testing ✅ COMPLETE
- [x] Test app cold start in airplane mode
- [x] Test creating session while offline
- [x] Test measuring group while offline
- [x] Test sync when coming back online
- [x] Test error handling when going offline mid-operation
