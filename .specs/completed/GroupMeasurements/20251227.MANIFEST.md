...
- ✅ Integration into `SessionDetailView` (GroupMeasurementFlow sheet)
- ✅ "Measure Group" / "View Measurement" button on GroupEntry cards
- ✅ Cancel confirmation dialog with `hasUnsavedWork` check
- ✅ Delete button in MeasurementReviewView
- ✅ Image upload endpoint and iOS integration
- ✅ Success overlay and loading states

**Files Modified:**
- `SessionDetailView.swift` - Added `measurementGroupEntry` state and sheet presentation
- `GroupEntryRow.swift` - Added `onMeasureTap` callback with measurement status display
- `GroupMeasurementFlow.swift` - Added cancel confirmation
- `MeasurementReviewView.swift` - Added delete button, success overlay
- `GroupMeasurementsController.cs` - Added image upload endpoint
...
- ✅ Created `LocalGroupMetricsCalculator.swift` with pure Swift implementation
- ✅ Full metrics: extremeSpread (CTC/ETE), meanRadius, H/V spread, stdDevs, CEP50
- ✅ MOA conversion utilities (`inchesToMoa`, `moaToInches`)
- ✅ Formatted string helpers for display

#### Phase 4b: Offline Measurement ViewModel ✅
- ✅ `QuickMeasureGroupViewModel` supports both online and offline modes via `SessionContext` enum
- ✅ `calculateAndSaveOffline()` uses `LocalGroupMetricsCalculator` for on-device calculations
- ✅ Saves to `PendingGroupMeasurement` and `PendingGroupEntry` for offline storage
- ✅ Images stored via `offlineQueue.queueImage()` with unique filenames

#### Phase 4c: UI Integration for Offline ✅
- ✅ `PendingDataEntryForms.swift` has `GroupMeasurementFlow` integration
- ✅ `PendingSessionDetailView.swift` shows `QuickMeasureGroupFlow` and `measurementSummary()`
- ✅ Measurement data displayed on pending group cards
- ✅ Same UI experience for online and offline modes

#### Phase 4d: Sync Enhancement ✅
- ✅ `OfflineSessionQueue.syncSession()` handles measurement sync (lines 417-485)
- ✅ Creates measurement on server via `GroupMeasurementService.createMeasurement()`
- ✅ Uploads original and annotated images via `ImageService`
- ✅ Graceful handling of partial failures (logs warnings, continues sync)

**Key Files:**
- `LocalGroupMetricsCalculator.swift` - On-device calculation engine (330 lines)
- `QuickMeasureGroupViewModel.swift` - Unified online/offline measurement workflow
- `OfflineSessionQueue.swift` - Full measurement sync with image upload
- `PendingSession.swift` - `PendingGroupMeasurement` model with image references
...
│   ├── Controllers/
│   │   ├── GroupMeasurementsController.cs  ✅ COMPLETE
│   │   └── TargetsController.cs            (FUTURE)
│   ├── Data/Entities/
│   │   └── GroupMeasurement.cs             ✅ COMPLETE
│   ├── DTOs/Sessions/
│   │   └── GroupMeasurementDtos.cs         ✅ COMPLETE
│   ├── Services/
│   │   ├── GroupMeasurementCalculator.cs   ✅ COMPLETE
│   │   └── TargetPdfGenerator.cs           (FUTURE)
│   └── Migrations/
│       └── 20251228160430_AddGroupMeasurement.cs ✅ COMPLETE
│
└── TrueDope-iOS/TrueDope-iOS/
    ├── Models/
    │   ├── GroupMeasurement.swift          ✅ COMPLETE
    │   └── PendingSession.swift            ✅ COMPLETE (has PendingGroupMeasurement)
    ├── Core/Services/
    │   ├── GroupMeasurementService.swift   ✅ COMPLETE
    │   └── OfflineSessionQueue.swift       ✅ COMPLETE (full sync with measurements)
    ├── Core/Utilities/
    │   └── LocalGroupMetricsCalculator.swift   ✅ COMPLETE
    └── Features/GroupMeasurement/
        ├── Views/
        │   ├── GroupMeasurementFlow.swift      ✅ COMPLETE
        │   ├── CaptureStepView.swift           ✅ COMPLETE
        │   ├── HoleMarkingView.swift           ✅ COMPLETE
        │   ├── MeasurementMetadataView.swift   ✅ COMPLETE
        │   ├── MeasurementReviewView.swift     ✅ COMPLETE
        │   ├── CalibrationStepView.swift       ✅ COMPLETE
        │   └── GroupStatsWidget.swift          ✅ COMPLETE
        ├── ViewModels/
        │   ├── GroupMeasurementViewModel.swift     ✅ COMPLETE
        │   └── QuickMeasureGroupViewModel.swift    ✅ COMPLETE (unified online/offline)
        └── Components/
            ├── HoleMarker.swift                ✅ COMPLETE
            ├── POAMarker.swift                 ✅ COMPLETE
            └── ZoomableImageView.swift         ✅ COMPLETE
```
...
| 2026-01-04 | 3.0 | Claude | Marked all phases COMPLETE - feature fully implemented |
